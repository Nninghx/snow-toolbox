...
def extract_items_id():
    ...
    if 'b23.tv' in url or 't.cn' in url or 'dwz.cn' in url or 'bit.ly' in url:
        result_label.config(text="正在解析短链接...")
        root.update()  # 刷新UI显示状态
        url = resolve_url(url)
    ...
    if match:
        ...
        result_label.config(text=f"提取结果: {items_id}")
        # 复制到剪贴板
        root.clipboard_clear()
        root.clipboard_append(items_id)
        messagebox.showinfo("成功", f"已提取 itemsId: {items_id}\n\n已自动复制到剪贴板")
    else:
        result_label.config(text="未找到 itemsId 参数")
...
# 标题
title_label = tk.Label(root, text="B站商品链接ID提取", font=("Microsoft YaHei", 16, "bold"))
